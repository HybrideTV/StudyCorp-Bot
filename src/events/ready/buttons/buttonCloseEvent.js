       ...
       //LISTE DES BOUTONS
        client.on('clickButton', async(button) => {
            if (button.id === 'closeTicket') {
                ...
                if(data) {                    
                    if(button.channel.name.includes("commande")){
                        await fs.writeFile(`./transcripts/commandes/${channelnamefile}-${fileDateComplete}.html`, data).catch(err => console.log(err));

                    }else{
                        await fs.writeFile(`./transcripts/${channelnamefile}/${channelnamefile}-${fileDateComplete}.html`, data).catch(err => console.log(err));
                    }
                    let guildElement = document.createElement('div');
                    let guildText = document.createTextNode(button.guild.name);
                    guildElement.appendChild(guildText);
                    if(button.channel.name.includes("commande")){
                        await fs.appendFile(`./transcripts/commandes/${channelnamefile}-${fileDateComplete}.html`, guildElement.outerHTML).catch(err => console.log(err));

                    }else{
                        await fs.appendFile(`./transcripts/${channelnamefile}/${channelnamefile}-${fileDateComplete}.html`, guildElement.outerHTML).catch(err => console.log(err));
                    }
                    messages.forEach(async message => {
                        ...
                        if(button.channel.name.includes("commande")){
                            await fs.appendFile(`./transcripts/commandes/${channelnamefile}-${fileDateComplete}.html`, parentContainer.outerHTML).catch(err => console.log(err));
    
                        }else{
                            await fs.appendFile(`./transcripts/${channelnamefile}/${channelnamefile}-${fileDateComplete}.html`, parentContainer.outerHTML).catch(err => console.log(err));
                        }
